...
public class Preferences {
  ...
  static protected void init(String commandLinePrefs) {
    ...
    while (e.hasMoreElements()) {
      ...
      if (key.endsWith(platformExt)) {
        ...
        System.out.println("value = "+ value+"actualKey ="+actualKey);
  ...
  public Preferences() {
    ...
    label = new JLabel(" (requires restart of ROBOTIS CM9)");//JLabel(_("  (requires restart of Arduino)"));
    box.add(label);
    pain.add(box);
    ...
    box.add(fontSizeField);//[ROBOTIS]2013-01-03 support for ROBOTIS CM9 as below.
    label = new JLabel(" (requires restart of ROBOTIS CM9)");//JLabel(_("  (requires restart of Arduino)"));
    box.add(label);
    pain.add(box);
    d = box.getPreferredSize();
    ...
    box.add(label);
    verboseCompilationBox = new JCheckBox("compilation");//JCheckBox(_("compilation "));
    box.add(verboseCompilationBox);//[ROBOTIS]2013-01-03 we do not use "Upload" methology...
    verboseUploadBox = new JCheckBox("download");//JCheckBox(_("upload"));
    ...
    //[ROBOTIS]2013-01-03 support for ROBOTIS CM9 as below.
    ...
    if (Base.isWindows()) {
      autoAssociateBox =//[ROBOTIS]2013-01-03 support for ROBOTIS CM9 as below.
        new JCheckBox("Automatically associate .ino files with ROBOTIS CM9");//JCheckBox(_("Automatically associate .ino files with Arduino"));
    ...
    label = new JLabel("(edit only when ROBOTIS CM9 is not running)");//JLabel(_("(edit only when Arduino is not running)"));
    pain.add(label);
    d = label.getPreferredSize();
    label.setForeground(Color.gray);
    label.setBounds(left, top, d.width, d.height);
...
//    }
  ...
  static public boolean getBoolean(String attribute) {//그냥 각 설정에 대해서 yes or no인지만 알아낸다.
	 // System.out.println("[ROBOTIS]getBoolean = "+attribute);
    String value = get(attribute); //, null);
    //System.out.println("[ROBOTIS]get(attribute) value = "+value);
