...
//[ROBOTIS] added to support ARM Cortex M3 based CM-900 Project
...
public class Sketch {
  ...
  public Sketch(Editor editor, String path) throws IOException {
	 // System.out.println("################I am new Sketch Structor##############");
    this.editor = editor;
...
//    }
    ...
    // 1. concatenate all .pde files to the 'main' pde 모든 pde파일을 하나의 파일로 사슬처럼 잇다(Concatenate)
...
//        code[0].setPreprocName(className + ".java");
    ...
    for (SketchCode sc : code) {
      if (sc.isExtension("c") || sc.isExtension("cpp") || sc.isExtension("h")) {
        ...
        System.out.println("then loop over the code[] and save each .java file = "+filename);
        try {
          System.out.println("save the file = "+buildPath+filename);
...
//  }
  ...
  public String build(boolean verbose) throws RunnerException {
	 //[ROBOTIS] just added to insert dummy parameter 
	//List<String> tempErrors = null;
    return build(tempBuildFolder.getAbsolutePath(), verbose, new ArrayList<String>()/*hack*/);
  }
   ...
   * @return null if compilation failed, main class name if not
   */
  Compiler compiler = null; //[ROBOTIS] move to outside of build method for using in uploader method 
  public String build(String buildPath, boolean verbose,
	  ...
	  //System.out.println("[ROBOTIS]Build excute: build path is = "+buildPath);
	...
	// run the preprocessor
	    ...
	    if(buildUsing == null){
	    	System.out.println("[ROBOTIS]No board is selected, check your board in tools menu");
	    	return null;
	    }
	    ...
	    {
	        //System.out.println("[ROBOTIS]********for(ARM)*********");
	        compiler = new ArmCompiler();
	    } else {
	    ...
	    if (compiler.compile(this, buildPath, primaryClassName, verbose, compileErrors)) {
	      //System.out.println("[Sketch]build() compileErrors : "+ compileErrors);
  ...
  protected boolean exportApplet(boolean verbose) throws Exception { //[ROBOTIS]change usingProgrammer -> verbose
    return exportApplet(tempBuildFolder.getAbsolutePath(), verbose); // we don't support external programmer.
  }

  String fullBuildBinaryPath = null;
  /**
   * Handle export to applet.
   */
  public boolean exportApplet(String appletPath, boolean verbose)//[ROBOTIS]change usingProgrammer -> verbose
    throws RunnerException, IOException, SerialException {

    prepare();
    //ROBOTIS
   // PandoraSerialUploader.setSerialForPandoraUploader(pandoraSerialProxy);
    
    // build the sketch
    editor.status.progressNotice(_("Compiling sketch..."));//스케치 컴파일 시작~!
    String foundName = build(appletPath, verbose,          //[ROBOTIS]change usingProgrammer -> verbose
    						new ArrayList<String>()/*hack*/); //[ROBOTIS] added
...
//    }
    ...
    editor.status.progressNotice(_("Uploading...")); //업로딩 시작
    
    fullBuildBinaryPath = appletPath+File.separator+foundName+".bin";
    //[ROBOTIS][CHANGE]
   /* System.out.println("[UPLOAD]appletPath"+appletPath+"foundName"+foundName); 
 ...
 /* public void SetFullBuildBinaryPath(String tmp){
	  fullBuildBinaryPath = tmp;
  }*/
  public String GetFullBuildBinaryPath(){
	  return fullBuildBinaryPath;
  }
  ...
  protected String upload(String buildPath, String suggestedClassName, boolean usingProgrammer)
   ...
   Uploader uploader = null;
    ...
    boolean success = uploader.uploadUsingPreferences(buildPath,
                                                      ...
                                                      usingProgrammer);
*/
    //[ROBOTIS] added to support CM-900
    ...
    System.out.println("[ROBOTIS]Loading via " + uploadProgram);

    //[ROBOTIS][START]임시땜빵코드 업로드 기능구현을 작업중....
  /*  if (uploadProgram.equals("dfu-util")) 
    ...
    */
   // uploader = new PandoraSerialUploader();
   // uploader = getSerialUploader();
    
    //System.out.println("[ROBOTIS] is compiled?"+compiler.sketchIsCompiled);
    boolean success = uploader.uploadUsingPreferences(buildPath,
                                                      suggestedClassName,
                                                      usingProgrammer);
    //[ROBOTIS] added to support CM-900
