...
//[ROBOTIS][CHANGE] add to support cm-900
#include "iwdg.h"
/**
 * [ROBOTIS][CHANGE] 2012-12-14 support user interrupt
  */
...
void vcomDataRxCb(void) {
  ...
  /* to do, not checking very carefully for edge cases. USUALLY,
  ...
  /* ROBOTIS magic number, {0x43,0x4D,0x39,0x58} is "CM9X"*/
  //TxDStringC("newBytes = "); TxDHex8C(newBytes);TxDHex8C(reset_state);TxDStringC("\r\n");
  uint8 chkBuf[4]={0,0,0,0}; //[ROBOTIS]add to initialize chkBuf[] as 0x0
  uint8 cmpBuf[4] = {0x43,0x4D,0x39,0x58};//{0x31, 0x45, 0x41, 0x46};
...
RESULT usbNoDataSetup(u8 request) {
    ...
    if (Type_Recipient == (CLASS_REQUEST | INTERFACE_RECIPIENT)) {
        ...
        case (SET_CONTROL_LINE_STATE):
            ...
            line_dtr_rts = new_signal & 0x03; // for usbGetDTR() usbGetRTS()
            //[ROBOTIS][CHANGE] cm-900 reset method is only when DTR signal is negative(new_signal == 0x2)
           // unsigned int target = (unsigned int)usbWaitReset | 0x1;
